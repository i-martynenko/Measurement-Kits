...
{
    public partial class Form_CalibrationTermoByLakeShore : Form
    {
        ...
        public Form_CalibrationTermoByLakeShore(Form_Menu menu)
        {
            InitializeComponent();
            Form_Menu = menu;
        ...
        private void SetupPlots() 
        {
            ...
            Plot1.Plot.XLabel("Channel_A");
            Plot1.Plot.YLabel("Channel_B");
            
            

            DataLoggerPlot2 = Plot2.Plot.Add.DataStreamerXY(10000);
            Plot2.Plot.XLabel("Index");
            Plot2.Plot.YLabel("Channel_A");
        ...
        private void Form_CalibrationTermoByLakeShore_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void CalibrationTermoByLakeShore_Load(object sender, EventArgs e)
        {
            check_channel_A.Checked = true;
            ...
            Form_CalibrationTermoByLakeShore_SizeChanged(null, null);
        }
        ...
        private async void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_isRunning)
                {
                    ...
                    _isRunning = true;
                    button3.Text = "Pause";
                    await Task.Run(() => MeasurementLoop(_cts.Token, check_channel_A.Checked, check_channel_B.Checked, checkBox_Wtite_time_in_file.Checked));
                }
                else
                {
                    // Пауза
        ...
        private async Task MeasurementLoop(CancellationToken token, bool channel_A,bool channel_B,bool read_time = false)
        {
            ...
            if (read_time)
            {               
                File.AppendAllText(filePath, $"Time\tChannel_A ({unit_A})\tChannel_B ({unit_B})\r\n");
            }
            else
            {
                File.AppendAllText(filePath, $"Channel_A ({unit_A})\tChannel_B ({unit_B})\r\n");
            }
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                if (elapsedMicroSec >= _timeStepMs) // наприклад, 50 мкс
                {
                    ...
                    double value_channel_A = 0;
                    double value_channel_B = 0;
                    if ( channel_A )
                    {
                        value_channel_A = _lakeshore.GetTemperature_K("A");
                    }
                    else
                    {
                        value_channel_A = _lakeshore.GetSensor("A");
                    }
                    if (channel_B)
                    {
                        value_channel_B = _lakeshore.GetTemperature_K("B");
                    }
                    else
                    {
                        value_channel_B = _lakeshore.GetSensor("B");
                    }
                    ...
                    AddTemperature(value_channel_A);
                    // Запис у файл
                    //string line = $"{DateTime.Now:HH:mm:ss.fff}\t{temp:E8}\t{rate:E8}\n";
                    string line;
                    if (read_time)
                    {
                        line = $"{time}\t{value_channel_A:E8}\t{value_channel_B:E8}\r\n";
                                  
                    }
                    else
                    {
                        line = $"{value_channel_A:E8}\t{value_channel_B:E8}\r\n";
                    }                    
                    ...
                    if (measureCounter % 3 == 0) // кожні 3 цикли
                    {
                        ...
                        {
                            label_TempNow.Text = $"P{value_channel_A:f}K";
                    ...
                    this.Invoke(new Action(() =>
                    {                        
                        DataLoggerPlot1.Add(value_channel_A, value_channel_B);
                        DataLoggerPlot2.Add(measureCounter, value_channel_A);
                        Plot1.Refresh();
                        Plot2.Refresh();                       
                    }));
                }
